...
def epoch(data, w, b, i):
    print("Epoch:", i)
    if i >= MAX_ITERS:
        return (w, b)
    fullyTrained = True
    numvars = len(data[0][0])
    for piece in data: # Data is a list of tuples (x - vector, y - int) | x = piece[0] | y = piece[1]
            a = activation(piece[0], w, b)
            y = piece[1]
            if y*a <= 0: # Checks if activation and y have contrasting signs
                for var in range(numvars):
                    w[var] = w[var] + y*piece[0][var]
                fullyTrained = False
                b += y
    if not fullyTrained:
        return epoch(data, w, b, i+1)
    return (w, b)
